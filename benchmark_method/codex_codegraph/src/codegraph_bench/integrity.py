...
SAFE_RUN_ID = re.compile(r"^codex-codegraph-(?:smoke-)?[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
...
def validate_run_id(run_id: str, *, smoke: bool | None = None) -> str:
    ...
    if not SAFE_RUN_ID.fullmatch(run_id):
        raise IntegrityError("mutation_refused: run_id is not a codex-codegraph run component")
    is_smoke = run_id.startswith("codex-codegraph-smoke-")
    if smoke is True and not is_smoke:
        raise IntegrityError("mutation_refused: smoke run_id must start with codex-codegraph-smoke-")
...
def validate_run_manifest(manifest: dict[str, Any], *, expected_run_id: str | None = None) -> dict[str, Any]:
    ...
    if manifest.get("arm") != "codex-codegraph" or manifest.get("protocol") != "codegraph-region-v1":
        raise IntegrityError("mutation_refused: run is not owned by the codex-codegraph arm")
    ...
    if task2_root_digest is not None and not re.fullmatch(
        r"[0-9a-f]{64}", str(task2_root_digest)
    ):
