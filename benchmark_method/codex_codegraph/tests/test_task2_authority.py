...
class CanonicalAuthorityTests(unittest.TestCase):
    def test_v18_uses_fresh_downstream_paths_and_pinned_v17_tail(self):
        ...
        self.assertEqual(
            ...
            benchmark.ROOT / ".benchmark-work/codegraph/v18/smoke-gate.json",
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.dict(
                ...
                with self.assertRaisesRegex(benchmark.RunnerError, "authority environment override"):
                    benchmark.score(argparse.Namespace(run_id="codex-codegraph-authority-test"))
    ...
    def test_predecessor_state_matrix_every_partial_combination_refuses(self):
    ...
    def test_prepare_accepts_complete_chain_before_mutable_work(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                mock.patch.object(
                    ...
                    "paths",
                    return_value={
                        "task2_evidence_root": root / "v5-root.json",
                        "task2_freeze_marker": root / "v5-marker.json",
                    },
                ),
                mock.patch.object(
                    benchmark,
                    "_task2_authority_chain",
                    return_value=chain,
                ),
                mock.patch.object(
                    benchmark,
    ...
    def test_prepare_refuses_partial_chain_before_mutable_work(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            chain = self._copied_history(root, present={0, 1, 2})
            with (
                mock.patch.object(benchmark, "load_config", return_value={}),
                mock.patch.object(
                    benchmark,
