...
class MutationBoundaryTests(unittest.TestCase):
    ...
    def test_run_id_must_be_one_safe_treatment_component(self):
        for value in ("../codex-codegraph-x", "codex-codegraph/x", ".", "baseline-old", "/tmp/x"):
            with self.subTest(value=value), self.assertRaises(IntegrityError):
                validate_run_id(value)
        self.assertEqual(validate_run_id("codex-codegraph-20260727"), "codex-codegraph-20260727")
        self.assertEqual(validate_run_id("codex-codegraph-smoke-20260727", smoke=True), "codex-codegraph-smoke-20260727")
    ...
    def test_symlinked_run_root_refuses_containment(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            outside.mkdir()
            (runs / "codex-codegraph-linked").symlink_to(outside, target_is_directory=True)
            with self.assertRaisesRegex(IntegrityError, "escapes .benchmark-runs|symlinked run roots"):
                resolve_run_root(repository, "codex-codegraph-linked")
...
class SchemaBoundaryTests(unittest.TestCase):
    def base_record(self):
        return {
            "run_id": "codex-codegraph-fixture",
            "arm": "codex-codegraph",
            "task_id": "task",
            "sample_id": 1,
